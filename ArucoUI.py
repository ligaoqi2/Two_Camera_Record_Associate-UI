...
class ArucoUI(QtWidgets.QWidget):
    def __init__(self, parent=None):
        ...
        # 使用年限判断
        if int(datetime.datetime.now().strftime('%Y')) > 2023:
            yearBox = QMessageBox(QMessageBox.Information, u'提示', u'您已到达使用年限，请联系南京邮电大学管理员...')
            yearBox.exec_()
            sys.exit(0)
    ...
    def end_record(self):
        ...
        # 生成同步视频
        gene_video(self.path_result, self.out1, self.out2)

        # delete cache
        shutil.rmtree(os.path.join(self.save_path, self.record_time, "came1"))
        shutil.rmtree(os.path.join(self.save_path, self.record_time, "came2"))
        os.remove(os.path.join(self.path_result, "came1.txt"))
        os.remove(os.path.join(self.path_result, "came2.txt"))
        os.remove(os.path.join(self.path_result, "associate.txt"))
    ...
    def dialogIp(self):
        ...
        if dialog.exec() == QDialog.Accepted:
            ...
            try:
                ...
                # Set default focus
                setTheFocusDistance(self.ip_config1, 1.00)
                setTheFocusDistance(self.ip_config2, 1.00)
...
class ThreadCamera(QThread):
    ...
    def run(self) -> None:
        ...
        while True:
            ...
            # start record thread
            if record_flag and isinstance(sub_sub_thread, ThreadRecord):  # instance()判断是否初始化
                sub_sub_thread.start()

            elif not record_flag and isinstance(sub_sub_thread, ThreadRecord):
...
def gene_video(path_result, out1, out2):
    ...
    progress_dialog.setWindowTitle("请稍等")
    progress_dialog.setLabelText("正在同步时间戳...")
    progress_dialog.setMinimumDuration(5)
    progress_dialog.setWindowModality(Qt.WindowModal)

    progress_dialog.setRange(0, len(lines))

    for num in range(len(lines)):
        progress_dialog.setValue(num)
        frame1 = cv2.imread(os.path.join(path_result, "came1", lines[num].split(',')[0]))
        frame2 = cv2.imread(os.path.join(path_result, "came2", lines[num].split(',')[1]))
        out1.write(frame1)
        out2.write(frame2)

    else:
        progress_dialog.setValue(len(lines))
